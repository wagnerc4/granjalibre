...
def select_workers(dbObj, rq):
  return dbObj.getRows("""SELECT id, privilege, access, worker, phones, email
                          FROM workers ORDER BY worker;""")


def insert_worker(dbObj, rq):
  dbObj.execute("INSERT INTO workers VALUES(DEFAULT, %s, %s, %s, %s, %s, %s, FALSE);",
                (rq['usuario'], rq['telefonos'], rq['email'].strip(),
                 md5(rq['pass'].encode('utf-8')).hexdigest(), rq['_privilege'], rq['roles']))
  return select_workers(dbObj, rq)


def update_worker(dbObj, rq):
  dbObj.execute("""UPDATE workers SET worker=%s, phones=%s, email=%s,
                                    privilege=%s, access=%s WHERE id=%s;""", 
                (rq['usuario'], rq['telefonos'], rq['email'],
